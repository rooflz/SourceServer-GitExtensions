...
# with modifications by Jonathan Oliver
# ------------------------------------------------------------------------
...
sub GatherFileInformation {
	...
	# get unique identifier (SHA1) of initial repository commit
	if (!open($hProcess, "git rev-list --reverse master | head -1 |")) {
		::warn_message("Unable to query list initial commit for $sourcePath");
		return();
	}
